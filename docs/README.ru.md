...
- 🤖 Поддержка моделей Groq (Примеры):
  - **Llama 3 70B:** Мощная модель общего назначения от Meta (Контекст: 8192 токена). Отлично подходит для сложных задач, требующих глубокого понимания и генерации текста.
  - **Llama 3 8B:** Быстрая и эффективная модель от Meta (Контекст: 8192 токена). Хороша для повседневных задач, чат-ботов и быстрого ответа.
  - **Llama 3.1 8B Instant:** Улучшенная версия 8B от Meta (Контекст: 131072 токена). Обеспечивает значительно больший контекст для работы с длинными документами или диалогами.
  - **Llama 3.3 70B Versatile:** Улучшенная универсальная 70B от Meta (Контекст: 32768 токенов). Предлагает увеличенный контекст по сравнению со стандартной 70B.
  - **Mixtral 8x7B:** Модель "Смесь Экспертов" от Mistral AI (Контекст: 32768 токенов). Эффективна для широкого круга задач благодаря своей архитектуре.
  - **Gemma2 9B IT:** Эффективная инструктивная модель от Google (Контекст: 8192 токена). Оптимизирована для следования инструкциям и генерации текста.
  - **Whisper Large V3:** Транскрипция аудио от OpenAI (Примечание: Ввод аудио пока не реализован в плагине).
