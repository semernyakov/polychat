...
- 🤖 Поддержка моделей Groq (Примеры):
  - **Llama 3 70B**: Мощная модель для сложных задач.
  - **Llama 3 8B**: Быстрая и эффективная модель.
  - **Mixtral 8x7B**: Модель "Смесь Экспертов", универсальна.
  - **Gemma 2 9B**: Эффективная модель от Google.
  - **Whisper Large V3**: Для транскрипции аудио (Примечание: Ввод аудио пока не реализован в плагине).
